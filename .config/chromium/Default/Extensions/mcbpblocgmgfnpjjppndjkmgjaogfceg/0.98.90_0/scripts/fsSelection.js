...
if (typeof this.FireShotAddon == "undefined") {
    //noinspection ThisExpressionReferencesGlobalObjectJS
    this.FireShotAddon = {};
}

//noinspection ThisExpressionReferencesGlobalObjectJS
this.FireShotAddon.FSSelector =  function(options) {
    ...
    function getElementRect(elem, root) {
        var box = elem.getBoundingClientRect(),
            ...
            scrollTop = wnd.pageYOffset || docElem.scrollTop || body.scrollTop,
            scrollLeft = wnd.pageXOffset || docElem.scrollLeft || body.scrollLeft,
    ...
    function selectElementMouseMove(e) {
        ...
        fTimeout = setTimeout(function () {
            ...
            selectedElement = getScrolledElementAt(e.pageX, e.pageY) || doc.elementFromPoint(e.pageX - docBody.scrollLeft, e.pageY - docBody.scrollTop);
        ...
        if (e.button == 0) {
            ...
            function wrapperMouseMove(e) {
                if (e.constructor.name === 'TouchEvent' && e.touches.length === 1) {
                    ...
                    if (e.pageX === e.clientX && doc.body.scrollLeft > 0) {
                        e = {pageX : e.pageX, pageY: e.pageY, clientX: e.pageX - doc.body.scrollLeft, clientY: e.pageY - doc.body.scrollTop};
                    }
                }
            ...
            wrapper.addEventListener('touchmove', wrapperMouseMove, false);
            doc.addEventListener('mouseup', wrapperMouseUp, false);
            doc.addEventListener('touchend', wrapperMouseUp, false);
            doc.addEventListener('touchcancel', wrapperMouseUp, false);
        }
        e.preventDefault();
        return true;
    }
    ...
    function autoScroll(e) {
        ...
        shift = {dx: docBody.scrollLeft, dy: docBody.scrollTop};
        ...
        if (e.clientX < 100 && prevx > e.pageX)
            docBody.scrollLeft -= (100 - e.clientX) / speed;

        if (e.clientY < 100 && prevy > e.pageY)
            docBody.scrollTop -= (100 - e.clientY) / speed;

        var dX = wnd.innerWidth - e.clientX;
        if (dX < 100 && prevx < e.pageX)
            docBody.scrollLeft += (100 - dX) / speed;

        var dY = wnd.innerHeight - e.clientY;
        if (dY < 100 && prevy < e.pageY)
            docBody.scrollTop += (100 - dY) / speed;

        prevx = e.pageX;
        prevy = e.pageY;

        shift.dx = docBody.scrollLeft - shift.dx;
        shift.dy = docBody.scrollTop - shift.dy;
    ...
    return {
        makeSelection: function (onSelectedCB) {
            ...
            wrapper.addEventListener('touchstart', wrapperMouseDown, true);
...
//noinspection ThisExpressionReferencesGlobalObjectJS
this.FireShotAddon.FSSelectionHint =  function(docElement) {
    var
    ...
    return {
        show: function() {
            ...
            holder.style.cssText = "display:block; opacity:0; border:1px solid #666; background:#fff; z-index:2147483647; font-family: Tahoma; font-size:20px; color: #000; margin:0; padding:10px; width:100%;  left:0px; right:0px; top:0px; position:fixed";
            holder.innerHTML = "<b>HINT:</b> Hold the CTRL key to capture specific HTML element or scrolling area. &nbsp;";
